...
if __name__ == "__main__":
    # 原始数据路径（DFT参考）
    xyz_ref = "/home/giga/BIG/data/3M_F/3/scan/3_500k_NPT_opt_rot9.xyz"
    energy_true, force_true, pos_true = get_forces(xyz_ref, prefix="REF_", index=":")

    # 所有预测路径和前缀，添加更多模型只需在此添加路径即可
    model_paths = [
        ("/home/giga/BIG/data/3M_F/3/scan/3_500k_NPT_opt_rot9_out.xyz", "MACE_"),
        # ("/home/giga/BIG/data/3M_F/3/scan/3_500k_NPT_opt_rot9_out2.xyz", "MACE_"),
        # 添加更多模型时在此添加路径和前缀
    ...
    with plt.style.context(["science", "grid"]):
        ...
        ax[2].plot(energy_true, "k-", label="DFT")
        ...
        for i, (path, prefix) in enumerate(model_paths):
            ...
            try:
                ...
                ax[2].plot(energy_pred, "+", label=label)
        ...
        ax[2].set_title("Energy vs Frame")
        ax[2].legend(loc="best")

        plt.tight_layout()
        plt.savefig("force_energy.png")
        plt.show()
