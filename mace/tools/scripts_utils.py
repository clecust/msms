...
def extract_config_msmacecso_model(model: torch.nn.Module) -> Dict[str, Any]:
    ...
    config = {
        ...
        "r_min": model.r_min.item(),
        "r_mid": model.r_mid.item(),
        "xc": model.xc,
        ...
        "long_max_ell": model.long_spherical_harmonics._lmax,  # pylint: disable=protected-access
        ...
        "long_node_feats_irreps": model.long_node_feats_irreps,
...
def extract_config_macecso_model(model: torch.nn.Module) -> Dict[str, Any]:
    if model.__class__.__name__ != "ScaleShiftMACECSO":
        return {"error": "Model is not a ScaleShiftMACECSO model"}
...
def extract_config_msmace_model(model: torch.nn.Module) -> Dict[str, Any]:
    if model.__class__.__name__ != "ScaleShiftMSMACE":
        return {"error": "Model is not a ScaleShiftMSMACE model"}
...
def get_loss_fn(
    ...
    if args.loss == "weighted":
        ...
        loss_fn = modules.WeightedEnergyMeanForcesLoss(
            energy_weight=args.energy_weight, forces_weight=args.forces_weight
        )
...
def get_params_options(
    ...
    # args.d3_train
    # and args.finetune
    #     and has_dftd_submodule(model, "dispersion_correction")
    # ):
    #     logging.info(f"Only model.dispersion_correction.prefix will be trained")
    #     params_dict = {}
