...
class D3CSO_Calculator_f(nn.Module):
    ...
    def forward(
        ...
        # virial
        node_virials = e6.new_zeros((batch.shape[0], 3, 3))
        if compute_virials:
            ...
            if not self.bidirectional:
                ...
                # node_virials.index_add_(0, col, virials)
                node_virials *= 2.0
            else:
                # 双向图
                node_virials.index_add_(0, row, virials)
        ...
        # 初始化力张量
        force = torch.zeros((batch.size(0), 3), device=dij.device, dtype=dij.dtype)
        # 聚合节点能量
        node_g = e6.new_zeros((batch.shape[0], 1))
        ...
        return (
            ...
            2.0 * force * self.d3_autoev / self.d3_autoang,
            -node_virials * self.d3_autoev,
        )
