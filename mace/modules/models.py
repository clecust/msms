...
@compile_mode("script")
class ScaleShiftMSMACE(torch.nn.Module):
    ...
    def forward(
        ...
        long_edge_attrs = self.long_spherical_harmonics(vectors)
        ...
        for i, (interaction, product, readout) in enumerate(
            ...
            node_feats, sc = interaction(
                ...
                long_edge_index=data["edge_index"],
                first_layer=(i == 0),
                lammps_class=lammps_class,
                lammps_natoms=lammps_natoms,
            )
        ...
        return {
            ...
            "node_energy": node_energy,
            "interaction_energy": inter_e,
            "forces": forces,  # in mliap, forces is none !
            "edge_forces": edge_forces,  # in mliap, edge_forces.shape [edge_index,3] is changed with iter,
            "virials": virials,
            "stress": stress,
            "atomic_virials": atomic_virials,
            "atomic_stresses": atomic_stresses,
...
class ScaleShiftMSMACECSO_bak(torch.nn.Module):
    ...
    # 问题，可能出在最后cso汇聚时，用的edge_index[0] 汇聚到raw的ghost里，而不是col的local里，导致缺失部分色散力，从而密度发散；理论上，现在应该没问题了？
    def __init__(
        self,
        r_max: float,
        r_mid: float,  # add
        ...
        self.register_buffer(
            "r_mid", torch.tensor(r_mid, dtype=torch.get_default_dtype())
        )
        self.register_buffer(
...
class ScaleShiftMACECSO(MACE):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(**kwargs, r_max=r_min) # 强制父类MACE的截断使用r_min，正确做法
        self.register_buffer(
            "r_min", torch.tensor(r_min, dtype=torch.get_default_dtype())
        )
        # 重新赋值
    ...
    def forward(
        ...
        if is_lammps:
            node_disp = node_disp[: lammps_natoms[0]]
        node_inter_es = self.scale_shift(node_inter_es, node_heads)+ node_disp 
        inter_e = scatter_sum(node_inter_es, data["batch"], dim=-1, dim_size=num_graphs)
...
class ScaleShiftMSMACECSO(torch.nn.Module):
    # 主要来自　ScaleShiftMACECSO　的复制，即使用　　自动微分的方式求算cso的力
    def __init__(
        self,
        r_max: float,
        r_mid: float,  # add
...
@compile_mode("script")
class ScaleShiftMSMACECSOR(torch.nn.Module):
    # libtorch-mace-lammps的接口没有问题；　但是基于向量求导的mliap-lammps接口出现问题！！
    # 采用解析解的方式，太复杂，而且难以平衡mace-lammps和mliap-lammps的接口，另外还有Pytroch训练的接口
    # libtorch-mace: 采用自动微分的方法；　而mliap-lammps采用解析解（edge_forces为负梯度）,且返回的色散能量是没有vec和length的梯度关联的(两个都要取消！！有一个都不行)；　实现两个接口以及pytroch的统一
    ...
    def __init__(
        ...
        inter = RealAgnosticResidualInteractionBlockMSR(
            node_attrs_irreps=node_attr_irreps,
            node_feats_irreps=node_feats_irreps,
            edge_attrs_irreps=sh_irreps,
            edge_feats_irreps=edge_feats_irreps,
    ...
    def forward(
        ...
        node_feats = self.node_embedding(data["node_attrs"])
        ### R
        # index[0] 包括ghost的索引；而index[1]仅使用local的索引(mace默认)；但维度一致，应该是local和ghost的映射
        # vectors.requires_grad_(True)
        print(data.keys())
        # print(data["edge_index"][0].shape, data["edge_index"][0].max())
        # print(data["edge_index"][1].shape, data["edge_index"][1].max())
        ...
        # r_mid
        rmid_mask = lengths_r.squeeze(1) < self.r_mid
        long_edge_index = edge_index_r[:, rmid_mask]
        long_edge_attrs = self.long_spherical_harmonics(vectors_r[rmid_mask])
        long_edge_feats = self.long_radial_embedding(
            lengths_r[rmid_mask],
            data["node_attrs"],
            long_edge_index,
        ...
        # r_min
        rmin_mask = lengths_r.squeeze(1) < self.r_min
        edge_index = edge_index_r[:, rmin_mask]
        edge_attrs = self.spherical_harmonics(vectors_r[rmin_mask])
        edge_feats = self.radial_embedding(
            lengths_r[rmin_mask],
            data["node_attrs"],
            edge_index,
