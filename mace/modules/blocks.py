...
class EquivariantProductBasisBlock(torch.nn.Module):
    def __init__(
        ...
        lmax = o3.Irreps(target_irreps).lmax
        dim = o3.Irreps(node_feats_irreps).count(o3.Irrep(0, 1))
        irreps_mid = (o3.Irreps.spherical_harmonics(lmax) * dim).sort()[0].simplify()
...
@compile_mode("script")
class RealAgnosticResidualInteractionBlockMS(torch.nn.Module):
    ...
    def _setup(self) -> None:
        ...
        self.skip_tp = FullyConnectedTensorProduct(
            self.node_feats_irreps,
            self.node_attrs_irreps,
            self.hidden_irreps,  # irreps_out hidden_irreps
            cueq_config=self.cueq_config,
        )
        self.reshape = reshape_irreps(self.irreps_out, cueq_config=self.cueq_config)
    ...
    def forward(
        ...
        long_message = scatter_sum(
            src=long_mji, index=long_edge_index[1], dim=0, dim_size=num_nodes
        )  # [n_nodes, irreps]
        long_message = self.long_linear(long_message)
        ...
        sc = self.truncate_ghosts(sc, n_real)
        return (
            self.reshape(message / self.avg_num_neighbors),
            sc,
        )  # [n_nodes, channels, (lmax + 1)**2]


@compile_mode("script")
class RealAgnosticResidualInteractionBlockMS_noL(torch.nn.Module):
    # 没有远程的计算，作为第一层，但保留接口
    def __init__(
        self,
        node_attrs_irreps: o3.Irreps,
